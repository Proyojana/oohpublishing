			...
			handler: function (){				
				...
				var conn = new Ext.data.Connection();
					conn.request({
						...
						method: 'POST',
						params : {action:2,currency_rate_gbp:currency_rate_gbp,currency_rate_from:currency_rate_from,currency_rate_to:currency_rate_to},
						success:function(response){
							obj = Ext.JSON.decode(response.responseText);
							Ext.Msg.alert('Message', obj.message); 
							currentForm.getForm().reset();
							Ext.getCmp('currencyrategrid').getStore().reload();
							Ext.getCmp('currencyratetab').layout.setActiveItem('currencyrategrid');
